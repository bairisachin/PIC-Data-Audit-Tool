...
### OutlookEmail.py
class EmailSender:
    def __init__(self, total_pass, total_fail, total_error):
        self.totalPass = total_pass
        self.totalFail = total_fail
        ...
        self._smtpUserEmail = 'PICAuditingTool@genmills.com'
...
class ExcelScraper:
    ...
    def compare_excel(self):
        ...
        for row in self.sourceSheet.iter_rows(min_row=6):
            ...
            if check_diff(row[self.headerMap["Key Features 4"]].value, self.scrapeSheet.cell(row=currRowIdx, column=self.scrapeSheetHeaderMap["Key Features 4"]).value):
                ...
                kf4Diff = get_diff(row[self.headerMap["Key Features 4"]].value, self.scrapeSheet.cell(row=currRowIdx, column=self.scrapeSheetHeaderMap["Key Features 4"]).value)
            ...
            if (correctProdNamePercent + correctDescPercent + correctKeyFeaturePercent >= 75): # >= 75% is the passing condition
                ...
                for cell in currRow: # Copy entire row from sourceSheet to republishSheet
                    ...
                    #     continue
                    if cell.value is not None and cell.column == self.headerMap["URLs"] + 1 and (cell.hyperlink is not None and cell.hyperlink.target is not None):
                        # print(cell.value)
                        rowValues.append(cell.hyperlink.target)
                        continue
    ...
    def load_static_excel_content(self):
        ...
        for idx, row in enumerate(staticRows):
            ...
            if self.urlList[idx-1] == '#PAGE NOT FOUND' or self.urlList[idx-1] == '#URL NOT FOUND' or self.urlList[idx-1] == '#ROBOT OR NOT PAGE' or self.urlList[idx-1] == '#NO PRODUCT DETAILS FOUND':
            ...
            try:
                if self.urlList[idx] != "#URL NOT FOUND":
                    ...
                    if h1Text == "Uh-oh..." or spanText == "This page could not be found.":
                        ...
                        productDesc = self.get_description(soup)
                        productKF = self.get_keyfeatures(soup)
                        ...
                        dataList.append([productName, productDesc, *productKF])

                    # Handle robot or not page blank cells
                    if dataList[-1] == ["","","","","","",""]:
                       if self.robot_or_not(soup) == 'Robot or human?':
                            self.urlList[idx] = '#ROBOT OR NOT PAGE'
                       else:
                            self.urlList[idx] = '#NO PRODUCT DETAILS FOUND'
    ...
    def robot_or_not(self, soup):
        try:
            robotOrNotText = soup.find('h2', {'id': 'ld_modalTitle_0'}).text.strip()
        except:
            robotOrNotText = ""
        return robotOrNotText
    ...
    def send_email(self, RECIPIENT, FILE_PATH):
        email = EmailSender(self.total_pass, self.total_fail, self.total_error)
...
class App:
    ...
    def Reset(self):
        ...
        elapsedTime_label.configure(text="Estimated time: ???")
    ...
    async def compare_generate_excel(self):
        ...
        # elapsedTime_label.configure(text=f'Estimated time: Approx. {(30*es.total_products)/60:.2f} minutes...')
        # root.update()
        ...
        elapsedTime_label.configure(text=f'Time taken: {elapsed_time/60:.2f} minutes')
    ...
    async def update_counter(self, es):
        ...
        while es.stopFlag:
            ...
            elapsedTime_label.configure(text=f'Estimated time: Approx. {(timePerProduct * (totalProducts - cnt))//60}:{(timePerProduct * (totalProducts - cnt)) % 60} Minutes...')
            root.update()
...
elapsedTime_label = Label(root, text="Estimated time: ???", fg="#000")
